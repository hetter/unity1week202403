...
#if UNITY_EDITOR
public class MstJobProgressMMAssetAssignment
{
[CLSCompliantAttribute(false)]
	public static MstJobProgress_MMItem[] CreateItemTable(List<Dictionary<string, string>> allItemValueRowList)
	{
		if (allItemValueRowList == null || allItemValueRowList.Count == 0)
			return null;
		int rowCount = allItemValueRowList.Count;
		MstJobProgress_MMItem[] items = new MstJobProgress_MMItem[rowCount];
		for (int i = 0; i < items.Length; i++)
		{
			items[i] = new MstJobProgress_MMItem();
